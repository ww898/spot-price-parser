...
namespace AwsPriceParser
{
  ...
  {
    private static readonly Regex ourAwsInstanceTypeRegex = new(
      @"^(?'series'[a-z]+)(?'generation'\d*)(?'options'[a-z\d]*)(-(?'parameter'[a-z\d]+))?\.(?'size'[a-z\d-]+)$",
      RegexOptions.Compiled);

    public static AwsInstanceType Parse(string instanceType) => TryParse(instanceType) ?? throw new InvalidOperationException($"Unsupported AWS EC2 instance name ${instanceType}");

    public static AwsInstanceType? TryParse(string instanceType)
    {
      var match = ourAwsInstanceTypeRegex.Match(instanceType.ToLowerInvariant());
      if (!match.Success)
        return null;
